...
# Constants
HAUST_TESTNET_RPC_URL = "https://rpc-testnet.haust.app"
CHAIN_ID = 1523903251
EXPLORER_URL = "https://explorer-testnet.haust.app/tx/0x"
WHAUST_CONTRACT = "0x6C25C1Cb4b8677982791328471be1bFB187687c1"
...
LANG = {
    'vi': {
        'title': 'WRAP HAUST - HAUST TESTNET',
        'info': 'Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
        ...
        'wrap_success': 'Wrap {amount} HAUST → WHAUST thành công!',
        'unwrap_success': 'Unwrap {amount} WHAUST → HAUST thành công!',
        'failure': 'Thất bại',
        'address': 'Địa chỉ',
        'amount': 'Số dư',
        'gas': 'Gas',
        ...
        'block': 'Khối',
        'balance': 'Số dư: {haust:.6f} HAUST | WHAUST: {whaust:.6f}',
        'balance_info': 'Số dư',
        'pausing': 'Tạm nghỉ',
        'seconds': 'giây',
        ...
        'input_amount': 'Nhập số HAUST (0.000001 - 999): ',
        ...
        'choose_action': 'Chọn hành động [1: HAUST → WHAUST | 2: WHAUST → HAUST | 3: Wrap ←→ Unwrap]: ',
        'wrap_only': 'Wrap HAUST → WHAUST',
        'unwrap_only': 'Unwrap WHAUST → HAUST',
    ...
    'en': {
        ...
        'wrap_success': 'Successfully wrapped {amount} HAUST → WHAUST!',
        'unwrap_success': 'Successfully unwrapped {amount} WHAUST → HAUST!',
        'failure': 'Failed',
        'address': 'Address',
        'amount': 'Balance',
        'gas': 'Gas',
        ...
        'total_cost': 'Total Cost',
        'block': 'Block',
        'balance': 'Balance: {haust:.6f} HAUST | WHAUST: {whaust:.6f}',
        ...
        'input_amount': 'Enter HAUST amount (0.000001 - 999): ',
        ...
        'choose_action': 'Choose action [1: HAUST → WHAUST | 2: WHAUST → HAUST | 3: Wrap ←→ Unwrap]: ',
        'wrap_only': 'Wrap HAUST → WHAUST',
        'unwrap_only': 'Unwrap WHAUST → HAUST',
...
def display_all_wallets_balances(w3: Web3, private_keys: List[str], language: str = 'en'):
    ...
    print(f"{Fore.CYAN}  Wallet | {'HAUST':<10} | {'WHAUST':<10}{Style.RESET_ALL}")
    print(f"{Fore.CYAN}  {'-' * 6} | {'-' * 10} | {'-' * 10}{Style.RESET_ALL}")
    
    contract = w3.eth.contract(address=Web3.to_checksum_address(WHAUST_CONTRACT), abi=contract_abi)
    for i, key in enumerate(private_keys, 1):
        address = Account.from_key(key).address
        haust_balance = check_balance(w3, address, None, language)
        whaust_balance = check_balance(w3, address, contract, language)
        print(f"{Fore.YELLOW}  {i:<6} | {haust_balance:>10.6f} | {whaust_balance:>10.6f}{Style.RESET_ALL}")
...
def display_token_balances(w3: Web3, address: str, contract, language: str = 'en'):
    haust_balance = check_balance(w3, address, None, language)
    whaust_balance = check_balance(w3, address, contract, language)
    print_message(f"    - {LANG[language]['balance'].format(haust=haust_balance, whaust=whaust_balance)}", Fore.YELLOW)
...
def connect_web3(language: str = 'en'):
    try:
        w3 = Web3(Web3.HTTPProvider(HAUST_TESTNET_RPC_URL))
...
def check_balance(w3: Web3, address: str, contract=None, language: str = 'en') -> float:
    if contract is None:  # Native HAUST
        try:
...
async def wrap_haust(w3: Web3, private_key: str, amount: int, proxy: str, language: str) -> bool:
    ...
    for attempt in range(CONFIG['MAX_RETRIES']):
        try:
            ...
            haust_balance = check_balance(w3, sender_address, None, language)
            whaust_balance = check_balance(w3, sender_address, contract, language)
            print_message(f"    - {LANG[language]['balance'].format(haust=haust_balance, whaust=whaust_balance)}", Fore.YELLOW)
            if haust_balance < CONFIG['MINIMUM_BALANCE'] or haust_balance < w3.from_wei(amount, 'ether'):
                print_message(f"  ✖ {LANG[language]['insufficient_balance'].format(balance=haust_balance, symbol='HAUST', required=max(CONFIG['MINIMUM_BALANCE'], w3.from_wei(amount, 'ether')))}", Fore.RED)
                return False
            ...
            tx_params = {
                'from': sender_address,
                'to': Web3.to_checksum_address(WHAUST_CONTRACT),
                'value': amount,
                'data': '0xd0e30db0',
                'nonce': nonce,
                'chainId': CHAIN_ID,
            ...
            if receipt is None:
                ...
                print_message(f"    - {LANG[language]['total_cost']}: {total_cost:.12f} HAUST", Fore.YELLOW)
                haust_balance = check_balance(w3, sender_address, None, language)
                whaust_balance = check_balance(w3, sender_address, contract, language)
                print_message(f"    - {LANG[language]['balance'].format(haust=haust_balance, whaust=whaust_balance)}", Fore.YELLOW)
                return True
...
async def unwrap_whaust(w3: Web3, private_key: str, amount: int, proxy: str, language: str) -> bool:
    account = Account.from_key(private_key)
    sender_address = account.address
    contract = w3.eth.contract(address=Web3.to_checksum_address(WHAUST_CONTRACT), abi=contract_abi)
    ...
    for attempt in range(CONFIG['MAX_RETRIES']):
        try:
            ...
            whaust_balance = check_balance(w3, sender_address, contract, language)
            print_message(f"    - {LANG[language]['balance'].format(haust=haust_balance, whaust=whaust_balance)}", Fore.YELLOW)
            if haust_balance < CONFIG['MINIMUM_BALANCE']:
                print_message(f"  ✖ {LANG[language]['insufficient_balance'].format(balance=haust_balance, symbol='HAUST', required=CONFIG['MINIMUM_BALANCE'])}", Fore.RED)
                return False
            if whaust_balance * (10 ** 18) < amount:
                print_message(f"  ✖ {LANG[language]['insufficient_balance'].format(balance=whaust_balance, symbol='WHAUST', required=w3.from_wei(amount, 'ether'))}", Fore.RED)
                return False
    ...
    try:
        for cycle in range(1, cycles + 1):
            ...
            if action == '1':  # Wrap only
                ...
                if await wrap_haust(w3, private_key, amount_wei, proxy, language):
                    successful_txs += 1
            
            elif action == '2':  # Unwrap only
                print_message(f"> {LANG[language]['unwrap_only']}", Fore.CYAN)
                if await unwrap_whaust(w3, private_key, amount_wei, proxy, language):
                    successful_txs += 1
